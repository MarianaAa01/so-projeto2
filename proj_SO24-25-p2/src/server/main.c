...
static void *client_thread(void *arg_struct)
{
  ...
  // loop para tar sempre a ler e a responder a requests de clients
  while (buffer[0] != 2)
  { // sai daqui quando o cliente quer dar disconnect
    ...
    cada vez que uma key for mudada, isso avisa logo todos os clientes que estão subscritos àquela key
    */
    // faz algo consoante ao OP_CODE recebido na request_message (buffer[0])
    switch (buffer[0])
    {
    case 3: // OP_CODE do subscribe
      ...
      if (kvs_subscribe(key) == 0) 
      { 
      write(resp_fd, &succeeded, sizeof(succeeded));
      //ADICIONAR AQUI O CLIENTE À HASHTABLE COM LINKED LIST DE CLIENTES SUBSCRITOS POR KEY
      }
      else
      {
        succeeded[0] = 0;
        write(resp_fd, &succeeded, sizeof(succeeded));
        write_str(STDERR_FILENO, "Unable to subscribe\n");
      }
      break;
    case 4: // OP_CODE do unsubscribe
      ...
      if (kvs_unsubscribe(unsubscribe_key) == 0)
      { // Successfully unsubscribed
        write(resp_fd, &succeeded, sizeof(succeeded));
        //RETIRAR AQUI O CLIENTE À HASHTABLE COM LINKED LIST DE CLIENTES SUBSCRITOS POR KEY
      }
      else
      {
        succeeded[0] = 0;
        write(resp_fd, &succeeded, sizeof(succeeded));
        write_str(STDERR_FILENO, "Unable to unsubscribe\n");
      }
      break;
    }
  }
...
int main(int argc, char **argv)
{
  ...
  // criar o FIFO (named pipe) de registo, por onde os clientes iniciam sessão no servidor
  ...
  // Criamos uma thread para controlar a comunicaçao cliente-servidor (parte 2 - exercicio 1.1, so 1 cliente)
  pthread_t thread_client;
  pthread_create(&thread_client, NULL, client_thread, (void *)&single_client_info);
